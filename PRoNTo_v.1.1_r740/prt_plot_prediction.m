...
if length(PRT.model(model).input.class) <=2,
    ...
    if fVvals_exist
        if fold == 1
            ...
            for f = 2:nfold+1
                ...
                func_valsc1 = fVals(targets==1);
                func_valsc2 = fVals(targets==2);
                yc1 = (f-1)*ones(length(func_valsc1),1);
                yc2 = (f-1)*ones(length(func_valsc2),1);
                if ~isempty(yc1), exyc1 = 1; end
                if ~isempty(yc2), exyc2 = 1; end
                if f==2
        ...
        % the future
        y = [0:nfold+1]';
        if (strcmp(PRT.model(model).input.machine.function,...
                'prt_machine_gpml') || strcmp(PRT.model(model).input.machine.function,...
                'prt_machine_gpclap'));
    ...
    set(axes_handle,'Color',[1,1,1])
    beep
    disp('Predictions plot not available for multiclass!')
end
