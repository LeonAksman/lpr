...
% $Id: prt_machine_svm_bin.m 741 2013-07-22 14:07:36Z mjrosa $
...
%LEON ADDED 20/1/2014
persistent firstRun;
if isempty(firstRun)
    pathstr         = fileparts(mfilename('fullpath'));
    
    if ispc
        addpath(fullfile(pathstr, 'libsvm-3.17/windows'));    
    else
        addpath(fullfile(pathstr, 'libsvm-3.17/matlab'));
    end
end
firstRun            = false;


SANITYCHECK=false; % can turn off for "speed". Expert only.
...
else %leon added for one-class SVM
    sgn   = 1;
end
